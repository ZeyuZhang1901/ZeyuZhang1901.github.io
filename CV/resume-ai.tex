...
data---where temporal leakage from pretraining silently inflates scores and the standard
pre/post check cannot see it: I prove when the inflation can be measured at all and correct
the score with one external reference (leakage-adjusted evaluation), measure the
contamination interpretably (\textbf{TimeSPEC}), and train it away with RL post-training
(\textbf{TEMPO}). First-author publications at EMNLP 2026 (Findings) and NeurIPS 2023,
with two more first-author papers under review or in submission (NeurIPS 2026, TMLR).
...
\begin{publist}
  \pub{P1}{Temporal Leakage in LLM Backtesting: Measurement, Validation, and Adjusted Scores}
    ...
    {\me, B.\ C.\ Stadie \;
     \href{https://arxiv.org/abs/2608.02985}{[arXiv:2608.02985]}
...
\story{Backtesting---asking a model historical questions under a cutoff date, as if it were forecasting
  ...
  silently: \textbf{temporal leakage}, post-cutoff knowledge from pretraining seeping into the model's
  reasoning, inflates backtest scores---and the standard pre/post audit cannot see it, because legitimate
  recency produces the same signature. My work treats this as one program: a \textbf{statistical theory}
  of what a backtest can reveal and how one reference restores a corrected score [P1], an
  \textbf{interpretable audit} and inference-time defense [P3], and a \textbf{training method} that
  instills temporal discipline [P2].}
\begin{bullets}
  ...
  \item \facet{Prove what evaluation requires [P1].} Proved the inflation is \emph{non-identifiable} from
        ...
        not certify a clean backtest, and four flagships fail the naive check on questions they cannot
        have memorized---and that one external reference restores measurement: a matched clean control
        (difference-in-differences) or a known cutoff (regression discontinuity), yielding a
        \textbf{leakage-adjusted score} with confidence intervals. Estimators recover leakage planted in
        twin models in dose, location, and per-question profile, return null on the clean twin, and,
        deployed on frontier models over a 1{,}646-question ForecastBench panel, detect one
        cutoff-localized signature while clearing five models whose apparent advantages were recency alone.
